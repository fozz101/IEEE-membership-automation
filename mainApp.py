...
def proceedPaymentWindow():
    ...
    months_options = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October",
                      "November", "December"]
    ...
    startBtn = tk.Button(newWindow, text="START", state=tk.DISABLED,
                         command=lambda: [script.mainWithPayment(excelFile.name, memberName.get(1.0, tk.END),
                                                                 memberID.get(1.0, tk.END),
...
def notProceedPaymentWindow():
    ...
    startBtn = tk.Button(newWindow, text="START", state=tk.DISABLED,
                         command=lambda: [script.mainNoPayment(excelFile.name, memberName.get(1.0, tk.END),
                                                               memberID.get(1.0, tk.END))])
...
paymentCheckbox = tk.Checkbutton(mainWindow,
                                 text="I want the script to proceed to the payment phase !", variable=cb, onvalue=1,
                                 offvalue=0)
